...
void VM::scanCb(const sensor_msgs::msg::LaserScan::SharedPtr scan)
{
  ...
  cv::Mat ROIR = img(cv::Rect(LENGTH/2, ROI, LENGTH/2-ROI, LENGTH/2-ROI));
  ...
  rdmin = sqrt(statsr.at<int>(1, 0)*(statsr.at<int>(1, 0))+(LENGTH/2-statsr.at<int>(1, 1)-statsr.at<int>(1, 3))*(LENGTH/2-statsr.at<int>(1, 1)-statsr.at<int>(1, 3)));
    ...
    if (p[4] < 20) continue; 
		cv::rectangle(img, cv::Rect(p[0]+ROI, p[1]+ROI, p[2], p[3]), cv::Scalar(0, 255, 255)), 2; // x,y,가로,세로 크기
    if(ldmin > sqrt((LENGTH/2-p[0]-p[2])*(LENGTH/2-p[0]-p[2])+(LENGTH/2-p[1]-p[3])*(LENGTH/2-p[1]-p[3]))){
      ldmin = sqrt((LENGTH/2-p[0]-p[2])*(LENGTH/2-p[0]-p[2])+(LENGTH/2-p[1]-p[3])*(LENGTH/2-p[1]-p[3]));
      lx = p[0];
      ly = p[1];
      lw = p[2];
      lh = p[3];
  ...
  for (int j = 1;j < cntr;j++) {
    ...
    if(rdmin > sqrt(p[0]*p[0]+(LENGTH/2-p[1]-p[3])*(LENGTH/2-p[1]-p[3]))){
      rdmin = sqrt(p[0]*p[0]+(LENGTH/2-p[1]-p[3])*(LENGTH/2-p[1]-p[3]));
      rx = p[0];
      ry = p[1];
      rw = p[2];
      rh = p[3];
  ...
  if(cntl == 1&&cntr != 1){
    cv::line(img, cv::Point(LENGTH/2, LENGTH/2), cv::Point(200, LENGTH/2), cv::Scalar(0, 255, 255), 3);
    cv::line(img, cv::Point(LENGTH/2, LENGTH/2), cv::Point(rx+LENGTH/2, ry+rh+ROI), cv::Scalar(255, 255, 0), 3);
    ltheta = 0;
    rtheta = atan((400-ROI-ry-rh)/(rx));
  }
  else if(cntr == 1&&cntl != 1){
    cv::line(img, cv::Point(LENGTH/2, LENGTH/2), cv::Point(600, LENGTH/2), cv::Scalar(255, 255, 0), 3);
    cv::line(img, cv::Point(LENGTH/2, LENGTH/2), cv::Point(lx+lw+ROI, ly+lh+ROI), cv::Scalar(0, 255, 255), 3);
    ltheta = atan((400-ROI-ly-lh)/(400-ROI-lx-lw));
    rtheta = 0;
  }
  ...
  if((300-lx-lw) == 0 && rx == 0) err = 0;
  if(180-RAD2DEG(ltheta)-RAD2DEG(rtheta)<15)err = 100;
